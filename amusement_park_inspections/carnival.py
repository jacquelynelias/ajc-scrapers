...
def getContactPhone(text):
	try:
		result = re.findall(r'(PERMIT CONTACT:\s)(.*?)(\(*\d{3}\)*[- ]*\d{3}[- ]*\d{4})', text, re.DOTALL|re.MULTILINE)
		return result[0][2]
	except Exception as E:
		return "error"
...
def getLocation(text):
	try: 
		result = re.findall(r'(?:Location:\s*)(.*?)((P\.O\.|\d|PO BOX).*?)(?:PHONE:|Inspection|PERMIT)', text, re.DOTALL|re.MULTILINE)
...
def getPermitNum(text):
	try: 
		result = re.findall(r'(AC\d{0,2}-\d{4})', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"
...
def getStateIDNum(text):
	try: 
		result = re.findall(r'([A-Z]{3}[- ]\d{4,5})', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"
...
def getInspD8(text):
	try: 
		result = re.findall(r'(?:Inspector:\s|Inspection Date:\s)(\d{2}\/\d{2}\/\d{4})', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"
...
def getPermExpD8(text):
	try: 
		result = re.findall(r'(?:AC\d{0,2}-\d{4}\s|Permit Expiration Date:\s)(\d{2}\/\d{2}\/\d{4})', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"
...
def getManufacturerD8(text):
	try: 
		result = re.findall(r'(?:Manufacture Date:\s*)(\S*?)(?:\n|Seating)', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"	
...
def getSeatingCapacity(text):
	try: 
		result = re.findall(r'(?:Seating Capacity:\s*)(.*?)\s*(?:Child Seating)', text, re.DOTALL|re.MULTILINE)
		return result[0]
	except Exception as E:
		return "error"	
...
def getViolations(text):
	try:
		result = re.sub(r'(Viol\.\s*\t*|Date\s*\t*|Correct By\s*\t*|Resolved\s*\t*|Comment\s*\t*)', '', text[0])
		print("result", result)
		if (result != "None\n"):
			stuff = re.findall(r'(.*?)((\d{1,4}/\d{1,4}/\d{4}\s*)(12[:(0]*\s*)(\d{1,4}/\d{1,4}/\d{4}\s*)(12[:(0]*\s*)(\d{1,4}/\d{1,4}/\d{4}\s*)(12[:(0]*\s*))([a-zA-z0-9 \"])*|None', result, re.DOTALL|re.MULTILINE)
			#print(stuff)
			return stuff
		else: 
			#print("result", result)
...
try:
	for match in re.findall(r'(?:Bering)(.*?)(INSPECTION DETAILS|\Z)', orig_file, re.DOTALL|re.MULTILINE):
		#initializes fields
		full_text = ''
		full_address = ''
		company_name=''
		...
		conditions = re.findall(r'(?:Conditions / Violations\s*)(.*?)(?:INSPECTION DETAILS|\Z)', full_text, re.DOTALL|re.MULTILINE)
		...
		ride_name=getRideName(equipment[0])
		...
		#Breaks apart conditions into the condition, dates and comment 
		if (condition_result != "None"):
			if(len(condition_result) > 1):
				if(len(condition_result[0])>1):
					condition=condition_result[0][0]
				if(len(condition_result[0])>3):
					viol_d8=condition_result[0][2]
				if(len(condition_result[0])>5):
					correct_d8=condition_result[0][4]
				if(len(condition_result[0])>7):
					resolve_d8=condition_result[0][6]
		else:
			condition = "None"
			
		
		row = []
		#Takes each header and appends to a list to then amend to the output file
		for l in headers:
			row.append(eval(l))
		amend_file(output_file_name, row)

		#write_file(output_file_name, out)
		#print("Record #: ", i)
		i = i+1
except Exception as e:
	exc_type, exc_obj, exc_tb = sys.exc_info()
